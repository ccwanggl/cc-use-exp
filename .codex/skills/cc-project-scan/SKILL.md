---
name: project-scan
description: 扫描当前仓库并生成或更新项目级 AGENTS.md，适用于显式 project-scan、项目认知初始化或刷新项目级 Codex 上下文的场景；默认补齐项目内最小 .codex 骨架。
---

# Project Scan

当用户明确要求 `project-scan`、扫描项目、生成项目级 `AGENTS.md`，或刷新项目级 Codex 上下文时，使用本技能。
...
7. 默认不生成 Dockerfile、docker-compose、restart 或 ignore 文件；只有用户明确追加脚手架需求时，才转交 `project-init` 的方式处理。
8. 完成后说明扫描结论、写入策略、已补齐的项目级 `.codex` 骨架，以及仍需用户补充的项目事实。
...
- 明确区分“扫描识别”与“待确认项”
- 明确说明本次是“新建 / 增量更新 / 首次全量重建 / 追加自动区块”中的哪一种
